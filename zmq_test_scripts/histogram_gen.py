...
print("got databse")
data = db.child("breakthrough-listen-sandbox").child("flask_vars").child("processed_observations").child("Energy-Detection").get().val()

runtimes = []
for key in data:
  if 'fine' in key:
    print(data[key]['processing_time'])
    runtimes.append(data[key]['processing_time'])

import numpy as np
stuff = np.array(runtimes)
np.save("runtimes.npy", stuff)